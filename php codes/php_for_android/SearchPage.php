...
//IN: start, align_likes, temperature, limit, userID OUT: boardID, imagePath, imageTags, boardLikes, end
  ...
  if(isset($userID)){
    ...
    if(isset($align_likes)){
        $query2 = "SELECT boardID,imagePath,imageTags,boardLikes, IF(likedUser LIKE '%/$userID/%', 'true', 'false')liked FROM BOARD_LIKEST WHERE tempID=$tempID AND imageTags LIKE '%$keyword%' LIMIT $start, $limit";
    }
    else{
        $query2 = "SELECT boardID, imagePath, imageTags, boardLikes, IF(likedUser LIKE '%/$userID/%', 'true', 'false')liked FROM BOARD_LATEST WHERE tempID=$tempID AND imageTags LIKE '%$keyword%' LIMIT $start, $limit";
    }
